# 附录 25：OpenAI Codex 源码架构深度解析

> 定位：**OpenAI Codex 本地运行时的整仓源码解析**（全文收录）。分析对象是 `openai/codex` 官方开源仓库（Rust Workspace），快照锁定 `main` 提交 `0ae94fd`（2026-08-28 提交，2026-08-29 生成，仓库入口见 [C-35]）；文中源码链接均指向固定提交，仓库快速演进不影响快照结论。全文区分"源码事实 / 设计解释 / 演进推断"三级证据。与附录 23（Pi，TypeScript）、附录 24（Claude Code 还原源码，TypeScript）互为对照——三套 Coding Agent 运行时对同类问题（Agent Loop、工具治理、沙箱审批、会话持久化、MCP 扩展）的不同工程答案，Codex 的差异点是 Rust 内核与跨平台沙箱纵深。第六篇（第 23–25 章）的通用机制是阅读框架。
...
## 25.1 项目定位：Codex 不是一个单一 CLI，而是一套本地 Agent 运行时
...
### 25.1.2 三个核心名词：Thread、Turn、Item
...
## 25.5 CLI 总入口：MultitoolCli 如何成为能力路由器
...
## 25.7 非交互 Exec：面向自动化的稳定外壳

### 25.7.1 与 TUI 的相同点和不同点
...
### 25.8.2 Thread、Turn、Item 的远程化表达
...
## 25.9 协议层：Op、EventMsg 与 Item 生命周期

### 25.9.1 为什么需要独立 protocol crate
...
## 25.10 ThreadManager 与 CodexThread：会话控制面的两级抽象

### 25.10.1 ThreadManager 的职责
...
### 25.11.5 事件发送与 canonical record
...
## 25.12 run_turn：Agent Loop 的真正心脏
...
## 25.13 ModelClient 与 ModelClientSession：把连接复用和轮次隔离同时做好
...
## 25.14 上下文构建：历史、AGENTS.md、环境、Skill 与动态状态

### 25.14.1 上下文不是聊天记录的简单截取
...
### 25.14.3 world state 与 Turn 环境快照
...
## 25.15 上下文压缩：从 token 超限处理到可恢复状态变换
...
## 25.17 ToolRegistry：能力注册、Hook 和生命周期的统一入口

### 25.17.1 Registry 不是 HashMap 的薄包装
...
## 25.18 ToolOrchestrator：审批、沙箱选择与重试的策略中心
...
## 25.20 Shell、Unified Exec 与 PTY

### 25.20.1 为什么 shell 执行不是一个 `Command::output()`
...
## 25.22 Code Mode：把工具能力投射到受控代码执行环境
...
## 25.23 PermissionProfile：把“允许什么”变成可计算的数据结构
...
## 25.25 Guardian：用独立模型审查降低人工摩擦，但保持 fail closed

### 25.25.1 Guardian 的源码定位
...
## 25.26 SandboxManager：把统一策略转换为平台特定隔离
...
### 25.26.4 Windows Restricted Token
...
## 25.31 Plugin：能力包、来源与统一归因

### 25.31.1 PluginCapabilitySummary
...
## 25.34 多 Agent 控制面：根线程树隔离、共享预算与 AgentPath

### 25.34.1 AgentControl 的范围
...
## 25.36 ThreadStore：存储中立的线程持久化接口
...
## 25.37 LocalThreadStore：JSONL 事实日志与 SQLite 查询投影
...
## 25.38 RolloutRecorder：追加日志的写入、flush 与容错

### 25.38.1 Canonical JSONL
...
## 25.39 SQLite State Runtime：查询、迁移与投影重建
...
## 25.42 配置系统：多层合并、约束与来源可解释性

### 25.42.1 配置 crate 的真实复杂度
...
## 25.44 认证：多种凭据来源与可恢复登录

### 25.44.1 AuthManager 的职责
...
## 25.46 Realtime Conversation：语音、文本与后台 Agent 的双向协作

### 25.46.1 实时会话不是普通 Turn 流
...
## 25.47 OpenTelemetry：从日志到跨边界 Trace
...
## 25.48 Analytics：产品事实与运行时遥测的分工

### 25.48.1 Analytics facts
...
## 25.50 测试金字塔：从纯函数到真实平台

### 25.50.1 为什么 Codex 需要多层测试
...
### 25.59.16 `app-server-protocol-noop-macros`
...
### 25.59.17 `app-server-test-client`
...
### 25.59.23 `codex-backend-openapi-models`
...
### 25.59.32 `cloud-tasks-mock-client`
...
### 25.59.34 `collaboration-mode-templates`
...
### 25.59.54 `exec-server/tests/support`
...
### 25.59.70 `external-agent-migration`
...
### 25.59.92 `response-debug-context`
...
### 25.59.113 `utils/redacted-string`
...
### 25.59.114 `utils/rustls-provider`
...
### 25.59.118 `utils/sandbox-summary`
...
### 25.59.119 `utils/sleep-inhibitor`
...
### 25.59.120 `utils/approval-presets`
...
### 25.59.122 `utils/output-truncation`
...
### 25.59.133 `thread-manager-sample`
...
### 25.59.136 `codex-experimental-api-macros`
...
### 25.60.20 Code Mode host 缺失或版本不匹配
...
### 25.60.21 子 Agent 创建超过并发或 rollout 预算
...
### 25.60.24 Realtime 将语音任务交给后台 Agent
...
### 25.60.26 WebSocket 采样失败回退 HTTP
