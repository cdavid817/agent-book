# 附录 16：主流 Agent 自进化/自优化系统全景

> 定位：**Agent 自我改进（自进化/自优化）赛道的全景调研报告**（全文收录，信息基准 2026-09，代表系统与论文入口见 [C-40]）。与相邻内容的分工：第 26 章讲前沿方向的克制判断（自进化 = 自动化的变更管理、后训练四道闸、三问判断框架），本附录是整个赛道的地图——两条互补主线：**按进化对象**分（经验上下文/记忆/技能工具/Prompt 工作流/模型参数自博弈/代码递归，16.4–16.12）与**按优化机制**分（推理期反思自纠错 16.8、Prompt 程序自动优化 DSPy 工具链 16.9），再加反馈信号分级与多目标函数（16.14）、风险治理、参考架构与落地路线。成熟度分层：推理期反思与 Prompt 程序优化最先落地、记忆与技能进化工程化中、工作流进化试点、模型与代码自进化仍以研究为主——名单会过期，这个分层判断与"反馈信号决定优化上限"的框架不过期。

---

## 16.1 先统一定义：什么才算 Agent 自进化
...
前面各节讲的是**跨任务改变长期能力**；这一类不同——它在一次任务或连续几次尝试内，靠反思、验证、重规划得到更好结果，是 16.3 版图里"最轻量、最先落地"的一层，也是构建任何自优化闭环的基础组件（对应第 3 章 Verifier、第 4 章 Reflexion）。

| 方法 | 一句话 | 优化的是 | 反馈来源 | 参考 |
|---|---|---|---|---|
| **Self-Refine** | 同一模型自演生成者/批评者/修改者，反馈→改写→再评 | 当前输出 | 模型自评（弱） | [arXiv 2303.17651](https://arxiv.org/abs/2303.17651) |
| **Reflexion** | 把失败原因转成语言反思写入 episodic memory，下轮带经验重试 | 下一次尝试 | 环境分数 + 语言化经验 | [arXiv 2303.11366](https://arxiv.org/abs/2303.11366) |
| **CRITIC** | 用外部工具验证而非只信自评（搜索/计算器/编译器/分类器） | 当前输出 | **外部工具**（强） | [arXiv 2305.11738](https://arxiv.org/abs/2305.11738) |
| **LATS** | 把执行表示为搜索树，在"思考-行动-反馈"分支间搜索+价值评估 | 执行轨迹 | 环境反馈 + 价值评估 | [arXiv 2310.04406](https://arxiv.org/abs/2310.04406) |

一条选用线：**Self-Refine 优化当前输出、Reflexion 优化下一次、CRITIC 引外部验证、LATS 搜更好轨迹**。四者都不改模型权重，是 Agent Loop 的基础件；它们提高单次/短期成功率，但不天然形成跨任务的能力积累（那要靠后面几类）。一个关键纪律与第 15 章一致：**自评是弱信号**——CRITIC 的外部工具验证之所以对 Coding Agent 尤其有效，正因为编译/测试是确定性反馈，远比"模型觉得自己错了"可靠（见 16.14 的反馈信号可靠性分级）。

## 16.9 Prompt 与 Agent 程序自动优化（DSPy 工具链）

这是当前**成熟度最高、投入产出比最好**的自优化方向——不靠人肉反复调 Prompt，而是把"任务程序 + 训练样本 + 评估指标"交给优化器自动搜索更好的指令、Few-shot 与模块组合。它是 16.7"Prompt/工作流进化"的工程主力，值得单列。

**DSPy** 是这条线的代表框架：开发者声明式定义任务程序，由 Optimizer 自动优化。常见优化器与同类研究：

| 优化器/方法 | 机制 | 适合 | 参考 |
|---|---|---|---|
| **MIPROv2**（DSPy） | 联合优化指令 + Few-shot 示例 + 组合，贝叶斯搜索 | 指令与示例一起调 | [MIPROv2](https://dspy.ai/api/optimizers/MIPROv2/) |
| **GEPA**（DSPy） | **反思式进化**：读评估器的自然语言反馈→反思失败模式→生成下一代 Prompt | 错误原因复杂、单分数难表达 | [GEPA](https://dspy.ai/getting-started/gepa-optimization/) |
| **OPRO** | 把历史候选及得分放进上下文，让 LLM 当黑盒优化器提新候选 | 快速黑盒搜索 | [arXiv 2309.03409](https://arxiv.org/abs/2309.03409) |
| **ProTeGi** | "文本梯度"描述当前 Prompt 的错误方向，Beam Search + Bandit 筛选 | 有明确失败方向 | [arXiv 2305.03495](https://arxiv.org/abs/2305.03495) |
| **PromptBreeder** | 进化算法维护 Prompt 种群，**连"如何变异"的 Mutation Prompt 也一起进化**（有限自指） | 研究进化式优化 | [arXiv 2309.16797](https://arxiv.org/abs/2309.16797) |
| **TextGrad** | 把复合系统表示为计算图，用自然语言反馈模拟反向传播，优化 Prompt/代码/多节点变量 | 复杂多节点 Agent Pipeline | [arXiv 2406.07496](https://arxiv.org/abs/2406.07496) |

工程选型速查：**单 Prompt** 用 GEPA/MIPROv2；**指令+Few-shot 一起调** 用 MIPROv2；**有丰富文本反馈** 用 GEPA；**多节点复合系统** 用 TextGrad；**黑盒快速搜索** 用 OPRO/ProTeGi。一条与第 6 章呼应的判断：这类工具把"提示工程"从手艺变成**可搜索、可回归的优化问题**——但前提是你有 16.14 说的可靠评估指标；指标错了，优化器只会更快地过拟合到错误目标。
...
**自优化质量最终取决于反馈信号，而不是优化算法名称**——再好的优化器，喂给它错误或嘈杂的信号，只会更快地过拟合到错误目标。信号按可靠性分级（与第 15 章 Scorer 三类、第 24 章验证信号同源）：

| 反馈类型 | 示例 | 可靠性 | 适用 |
|---|---|---:|---|
| 确定性验证 | 单元测试、编译、SQL 结果、文件 Diff | 很高 | Coding、数据、工具执行 |
| 环境结果 | 页面状态、游戏得分、任务完成标记 | 高 | Web、GUI、机器人 |
| 规则评分 | Schema、长度、引用、权限、合规 | 高 | 企业流程 |
| LLM Judge | Rubric、正确性、相关性、完整性 | 中 | 开放文本 |
| 文本批评 | 失败原因、修改建议 | 中 | GEPA、TextGrad（16.9） |
| 偏好比较 | A 优于 B | 中 | 风格、体验、DPO |
| 用户纠正 | 用户编辑、重做、拒绝 | 中高 | 个性化 |
| 自生成反馈 | 自我反思、自我评分 | 较低 | 低成本探索 |

优先级：**确定性验证 > 环境状态 > 人工标注 > 多评估器共识 > 单个 LLM Judge > Agent 自我评价**。反馈应结构化留痕（trace_id / evaluator_type / score / reason / evidence / suggested_action），才能被优化器与回归门禁复用。

### 目标函数：不能只看成功率

自优化的目标是**多目标函数**，而非单一成功率——否则优化器会用成本、稳定性或旧能力退化换取表面提升：

$$J = \text{Quality} - \lambda_1\text{Cost} - \lambda_2\text{Latency} - \lambda_3\text{SafetyRisk} - \lambda_4\text{Regression} - \lambda_5\text{Instability} + \lambda_6\text{Retention} + \lambda_7\text{Generalization}$$

至少同时跟踪八维：任务质量（Success/Pass@K/Rubric）、轨迹质量（工具选择准确率、无效步、重试）、效率（Token/成本/延迟）、稳定性（方差、重复一致性）、泛化（未见任务/跨模型/跨环境）、保持能力（旧任务回归、遗忘率）、安全（越权/泄漏/危险调用）、可解释性（优化证据、修改 Diff）。**晋升门禁**（对应第 15 章回归门禁、16.15 风险治理）：质量提升 ≥ 阈值 且 高优先级回归为 0 且 安全不下降 且 成本延迟不超预算 且 无明显过拟合；其中 skill 代码、workflow 拓扑、runtime 补丁、权限策略这四类高风险改动**强制人工审批**。
