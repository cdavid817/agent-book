# 附录 U：OpenAI Codex 源码架构深度解析

> 定位：**OpenAI Codex 本地运行时的整仓源码解析**（全文收录）。分析对象是 `openai/codex` 官方开源仓库（Rust Workspace），快照锁定 `main` 提交 `0ae94fd`（2026-08-28 提交，2026-08-29 生成，仓库入口见 [C-35]）；文中源码链接均指向固定提交，仓库快速演进不影响快照结论。全文区分"源码事实 / 设计解释 / 演进推断"三级证据。与附录 S（Pi，TypeScript）、附录 T（Claude Code 还原源码，TypeScript）互为对照——三套 Coding Agent 运行时对同类问题（Agent Loop、工具治理、沙箱审批、会话持久化、MCP 扩展）的不同工程答案，Codex 的差异点是 Rust 内核与跨平台沙箱纵深。第六篇（第 23–25 章）的通用机制是阅读框架。
...
## U.1 项目定位：Codex 不是一个单一 CLI，而是一套本地 Agent 运行时
...
## U.5 CLI 总入口：MultitoolCli 如何成为能力路由器
...
## U.7 非交互 Exec：面向自动化的稳定外壳

### U.7.1 与 TUI 的相同点和不同点
...
## U.9 协议层：Op、EventMsg 与 Item 生命周期

### U.9.1 为什么需要独立 protocol crate
...
## U.10 ThreadManager 与 CodexThread：会话控制面的两级抽象

### U.10.1 ThreadManager 的职责
...
## U.12 run_turn：Agent Loop 的真正心脏
...
## U.13 ModelClient 与 ModelClientSession：把连接复用和轮次隔离同时做好
...
## U.14 上下文构建：历史、AGENTS.md、环境、Skill 与动态状态

### U.14.1 上下文不是聊天记录的简单截取
...
### U.14.3 world state 与 Turn 环境快照
...
## U.15 上下文压缩：从 token 超限处理到可恢复状态变换
...
## U.17 ToolRegistry：能力注册、Hook 和生命周期的统一入口

### U.17.1 Registry 不是 HashMap 的薄包装
...
## U.18 ToolOrchestrator：审批、沙箱选择与重试的策略中心
...
## U.20 Shell、Unified Exec 与 PTY

### U.20.1 为什么 shell 执行不是一个 `Command::output()`
...
## U.22 Code Mode：把工具能力投射到受控代码执行环境
...
## U.23 PermissionProfile：把“允许什么”变成可计算的数据结构
...
## U.25 Guardian：用独立模型审查降低人工摩擦，但保持 fail closed

### U.25.1 Guardian 的源码定位
...
## U.26 SandboxManager：把统一策略转换为平台特定隔离
...
### U.26.4 Windows Restricted Token
...
## U.31 Plugin：能力包、来源与统一归因

### U.31.1 PluginCapabilitySummary
...
## U.34 多 Agent 控制面：根线程树隔离、共享预算与 AgentPath

### U.34.1 AgentControl 的范围
...
## U.37 LocalThreadStore：JSONL 事实日志与 SQLite 查询投影
...
## U.38 RolloutRecorder：追加日志的写入、flush 与容错

### U.38.1 Canonical JSONL
...
## U.39 SQLite State Runtime：查询、迁移与投影重建
...
## U.42 配置系统：多层合并、约束与来源可解释性

### U.42.1 配置 crate 的真实复杂度
...
## U.44 认证：多种凭据来源与可恢复登录

### U.44.1 AuthManager 的职责
...
## U.46 Realtime Conversation：语音、文本与后台 Agent 的双向协作

### U.46.1 实时会话不是普通 Turn 流
...
## U.47 OpenTelemetry：从日志到跨边界 Trace
...
## U.48 Analytics：产品事实与运行时遥测的分工

### U.48.1 Analytics facts
...
## U.50 测试金字塔：从纯函数到真实平台

### U.50.1 为什么 Codex 需要多层测试
...
### U.59.16 `app-server-protocol-noop-macros`
...
### U.59.17 `app-server-test-client`
...
### U.59.23 `codex-backend-openapi-models`
...
### U.59.32 `cloud-tasks-mock-client`
...
### U.59.34 `collaboration-mode-templates`
...
### U.59.54 `exec-server/tests/support`
...
### U.59.70 `external-agent-migration`
...
### U.59.92 `response-debug-context`
...
### U.59.113 `utils/redacted-string`
...
### U.59.114 `utils/rustls-provider`
...
### U.59.118 `utils/sandbox-summary`
...
### U.59.119 `utils/sleep-inhibitor`
...
### U.59.120 `utils/approval-presets`
...
### U.59.122 `utils/output-truncation`
...
### U.59.133 `thread-manager-sample`
...
### U.59.136 `codex-experimental-api-macros`
...
### U.60.20 Code Mode host 缺失或版本不匹配
...
### U.60.21 子 Agent 创建超过并发或 rollout 预算
...
### U.60.24 Realtime 将语音任务交给后台 Agent
...
> **使用提示**：与其他附录的分工——A 讲模型机制、B 讲方法论、C 记来源、D 列产品、E 辨异同、F 索引图版、G 详解 OTel、H 上手 DeepEval、I 评测观测平台选型、J 上手 Mem0、K 盘点 Coding Agent 赛道、L 盘点可观测赛道、M 盘点评估赛道、N 盘点 Memory 赛道、O 盘点自进化赛道、P 盘点多 Agent 赛道、Q 盘点 MCP 生态、R 盘点沙箱赛道、S 解析 Pi 源码、T 解析 Claude Code 源码、**U 解析 Codex 源码**、V 解析 OpenCode 源码。对照阅读：Agent Loop 与 Turn 闭环见篇二对第 3 章、压缩见 U.13 一带对第 5 章、工具编排见篇三对第 7 章、审批与沙箱见篇四对第 9/13 章、MCP 与多 Agent 见篇五对第 8/17 章、持久化与恢复见篇六对第 12 章、OTel 见篇七对第 14 章与附录 G。快照为固定提交 0ae94fd（[C-35]），二次开发前先核对上游变更；S/T/U 三份解析对读，是"同类问题不同答案"的最好教材。
