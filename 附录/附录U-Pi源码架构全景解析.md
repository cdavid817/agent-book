...
## U.13 工具调用：Schema 校验、钩子、并行、顺序与终止
...
## U.14 Steering、Follow-up、消息队列与交互控制
...
## U.16 Agent Harness：从通用循环到可恢复任务运行时
...
## U.19 Coding Agent 启动、CLI 解析与多模式路由
...
## U.21 Runtime 与 Services：会话替换和宿主注入边界
...
## U.22 会话格式：JSONL 树、分支、Fork、Clone 与上下文投影
...
## U.26 上下文文件、系统提示、Prompt Template、Skill 与 Theme
...
## U.30 Bash、PowerShell、进程取消与输出保护
...
## U.34 Print、JSON、RPC 与 SDK 嵌入模式
...
## U.35 远程协议：严格 CBOR、长度帧与运行时 Schema
...
## U.36 远程 Client、Server、会话租约与快照同步
...
## U.37 Telemetry：Span 生命周期、类型化 Schema 与隐私边界
...
## U.38 SQLite Session Backend、迁移与可选 FTS 搜索
...
## U.43 可扩展方案：MCP、子 Agent、权限层与企业集成
