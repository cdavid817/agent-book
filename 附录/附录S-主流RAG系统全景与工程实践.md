...
### S.7.6 AnythingLLM、Open WebUI、Kotaemon
...
## S.8 Code-first RAG 框架

### S.8.1 LangChain 与 LangGraph
...
### S.11.2 AWS Bedrock Knowledge Bases
...
### S.11.3 Azure AI Search / Microsoft Foundry
...
### S.11.4 Google Vertex AI RAG Engine
...
## S.42 Retrieval Text 与 Generation Text 分离
...
### S.52.2 共享 Collection + tenant_id 过滤
...
### S.111.6 Context Precision

相关 Chunk 是否排在不相关 Chunk 前面。

### S.111.7 Context Recall
...
### S.117.1 Answer Correctness

答案与标准答案、事实或业务规则是否一致。

### S.117.2 Answer Relevance

答案是否直接回应用户问题，而不是只复述背景。

### S.117.3 Faithfulness
...
## S.127 OpenTelemetry 与 OpenInference
...
### S.144.1 把向量数据库当成完整 RAG

忽略数据导入、解析、权限、重排、引用和评测。

### S.144.2 只调 Chunk Size
...
### S.144.5 所有查询都走 Agentic RAG

简单问题被过度规划，成本和不确定性显著增加。

### S.144.6 一开始就上 GraphRAG
...
> **使用提示**：与其他附录的分工——A 讲模型机制、B 讲方法论、C 记来源、D 列产品、E 辨异同、F 索引图版、G 详解 OTel、H 上手 DeepEval、I 评测观测平台选型、J 上手 Mem0、K 盘点 Coding Agent 赛道、L 盘点可观测赛道、M 盘点评估赛道、N 盘点 Memory 赛道、O 盘点自进化赛道、P 盘点多 Agent 赛道、Q 盘点 MCP 生态、R 盘点沙箱赛道、**S 盘点 RAG 赛道**、T 盘点 LLM Wiki 赛道、U 解析 Pi 源码、V 解析 Claude Code 源码、W 解析 Codex 源码、X 解析 OpenCode 源码。对照阅读：本附录与第 11 章是"手册对机制"的关系——逐环节对读：数据导入（第三部分）对第 11 章 2.1、解析分块（第四/五部分）对 2.2、Embedding（第六部分）对 2.3 与附录 A.8、向量存储与索引（第七/八部分）对 2.7、检索前处理（第九部分）对 2.4、混合检索与后处理（第十/十一部分）对 2.5、生成与引用（第十二部分）对 2.6、Agentic/GraphRAG（第十三部分）对 2.9、评估（第十四部分）对 2.8 与附录 H/M、观测治理（第十五部分）对第 14 章与附录 L、Memory 之辨对附录 E.1 与 N。信息基准 2026-08-31（[C-46]），发行前按附录 C 清单复核。
