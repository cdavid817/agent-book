...
> **使用提示**：与其他附录的分工——A 讲模型机制、B 讲方法论、C 记来源、D 列产品、E 辨异同、F 索引图版、G 详解 OTel、H 上手 DeepEval、I 评测观测平台选型、J 解析 Pi 源码、K 解析 Claude Code 源码、L 解析 Codex 源码、M 盘点 Coding Agent 赛道、N 盘点可观测赛道、O 盘点评估赛道、P 盘点 Memory 赛道、**Q 盘点自进化赛道**。对照阅读：定义与易混概念（Q.1–Q.2）对第 26 章"自进化 = 自动化的变更管理"、记忆型进化（Q.5）对第 10 章与附录 P、技能进化（Q.6）对第 6 章 Skill、模型参数进化（Q.8）对第 26 章后训练四道闸与轨迹回流、评测体系（Q.12）对第 15 章与附录 O、风险清单（Q.13）对第 13 章与第 26 章三问框架。信息基准 2026-08-30（[C-40]），发行前按附录 C 清单复核。
