...
### Q.3.1 Microsoft Agent Framework
...
### Q.3.4 Google Agent Development Kit
...
### Q.3.7 LlamaIndex AgentWorkflow
...
### Q.4.4 Anthropic Research 多 Agent 系统
...
### Q.5.1 Microsoft Foundry Agent Service
...
### Q.5.2 Gemini Enterprise Agent Platform
...
### Q.5.3 Amazon Bedrock AgentCore
...
### Q.6.8 Blackboard / Shared Workspace
...
### Q.9.6 Prompt Injection 与跨 Agent 污染
...
### Q.11.5 Offline、Online 与 Shadow 评估
...
## Q.13 什么时候不应该使用多 Agent

### Q.13.1 一个 Agent 加合适工具已经足够

如果普通函数、单 Agent 或确定性 Workflow 可以完成，应优先采用更简单方案。

### Q.13.2 任务高度串行且强依赖完整共享上下文
...
### Q.15.4 Agent Gateway 与 Agent Identity 成为基础设施
...
### Q.15.6 单 Agent + Skill 与多 Agent 融合
...
### Q.15.8 多 Agent 从在线对话转向 Durable Execution
...
> **使用提示**：与其他附录的分工——A 讲模型机制、B 讲方法论、C 记来源、D 列产品、E 辨异同、F 索引图版、G 详解 OTel、H 上手 DeepEval、I 评测观测平台选型、J 上手 Mem0、K 详解记忆晋升机制、L 盘点 Coding Agent 赛道、M 盘点可观测赛道、N 盘点评估赛道、O 盘点 Memory 赛道、P 盘点自进化赛道、**Q 盘点多 Agent 赛道**、R 盘点 MCP 生态、S 盘点沙箱赛道、T 盘点 RAG 赛道、U 盘点 LLM Wiki 赛道、V 盘点 Loop Engineering 赛道、W 解析 Pi 源码、X 解析 Claude Code 源码、Y 解析 Codex 源码、Z 解析 OpenCode 源码。对照阅读：多 Agent 定义与编排模式（Q.1/P.6）对第 17 章八拓扑与代价模型、框架盘点（Q.3）对附录 D.5、协议栈（Q.7）对第 8/18 章（MCP/A2A）、状态与记忆（Q.8）对第 18 章交接包与附录 O、沙箱权限（Q.9）对第 13 章与第 23 章 2.6、可观测（Q.10）对第 14/19 章与附录 M、评估（Q.11）对第 15 章与附录 N、该用不该用（Q.12–Q.13）对第 17/19 章三维选型与劝退判定。信息基准 2026-08-30（[C-41]），发行前按附录 C 清单复核。
