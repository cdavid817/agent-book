...
### O.4.20 评估器、LLM Judge 与人工标签质量指标
...
### O.8.3 Berkeley Function Calling Leaderboard
...
## O.9 Benchmark、框架与平台的关系

### O.9.1 DeepEval 与 SWE-bench 的关系
...
> **使用提示**：与其他附录的分工——A 讲模型机制、B 讲方法论、C 记来源、D 列产品、E 辨异同、F 索引图版、G 详解 OTel、H 上手 DeepEval、I 评测观测平台选型、J 解析 Pi 源码、K 解析 Claude Code 源码、L 解析 Codex 源码、M 盘点 Coding Agent 赛道、N 盘点可观测赛道、**O 盘点评估赛道**、P 盘点 Memory 赛道、Q 盘点自进化赛道。对照阅读：评估维度与指标清单（O.4）对第 15 章五层指标体系与第 14 章三类指标框架、代码级框架（O.6）中 DeepEval 的上手见附录 H、平台深评见附录 I、Benchmark（O.8）对第 15/24 章、指标设计与发布门禁（O.13）对第 15 章回归门禁、可观测联动对附录 N。信息基准 2026-08-30（[C-38]），发行前按附录 C 清单复核。
