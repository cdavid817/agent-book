...
## V.13 工具调用：Schema 校验、钩子、并行、顺序与终止
...
## V.14 Steering、Follow-up、消息队列与交互控制
...
## V.16 Agent Harness：从通用循环到可恢复任务运行时
...
## V.19 Coding Agent 启动、CLI 解析与多模式路由
...
## V.21 Runtime 与 Services：会话替换和宿主注入边界
...
## V.22 会话格式：JSONL 树、分支、Fork、Clone 与上下文投影
...
## V.26 上下文文件、系统提示、Prompt Template、Skill 与 Theme
...
## V.30 Bash、PowerShell、进程取消与输出保护
...
## V.34 Print、JSON、RPC 与 SDK 嵌入模式
...
## V.35 远程协议：严格 CBOR、长度帧与运行时 Schema
...
## V.36 远程 Client、Server、会话租约与快照同步
...
## V.37 Telemetry：Span 生命周期、类型化 Schema 与隐私边界
...
## V.38 SQLite Session Backend、迁移与可选 FTS 搜索
...
## V.43 可扩展方案：MCP、子 Agent、权限层与企业集成
