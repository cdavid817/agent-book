...
> **使用提示**：本附录与附录 B 的分工——A 回答"模型为什么是这样"（机制兜底），B 回答"工程方法叫什么、在哪一章"（方法论导航）。各附录分工：A 机制、B 方法论、C 来源、D 产品、E 易混概念辨析、F 图版索引、G OTel 详解、H DeepEval 实战、I 评测观测平台选型、J Pi 源码解析、K Claude Code 源码解析。都是工具书，不必通读，正文引用到时查阅即可。
