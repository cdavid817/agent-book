...
## K.11 主流 Agent 框架和产品的实现方式

主流系统并没有统一成一种架构，大体可以归纳为五条路线。

### K.11.1 路线一：Session + Long-term Store
...
### K.11.2 路线二：托管提取与 Consolidation
...
### K.11.3 路线三：独立 Memory Middleware
...
> **使用提示**：与其他附录的分工——A 讲模型机制、B 讲方法论、C 记来源、D 列产品、E 辨异同、F 索引图版、G 详解 OTel 与 Agent 观测、H 上手 DeepEval、I 评测观测平台选型、J 上手 Mem0、**K 详解记忆晋升机制**、L 盘点 Coding Agent 赛道、M 盘点可观测赛道、N 盘点评估赛道、O 盘点 Memory 赛道、P 盘点自进化赛道、Q 盘点多 Agent 赛道、R 盘点 MCP 生态、S 盘点沙箱赛道、T 盘点 RAG 赛道、U 盘点 LLM Wiki 赛道、V 盘点 Loop Engineering 赛道、W 解析 Pi 源码、X 解析 Claude Code 源码、Y 解析 Codex 源码、Z 解析 OpenCode 源码。对照阅读：分层模型（K.2）对第 10 章 CoALA 四分类与附录 E.1、晋升流程与价值判断（K.4–K.5）对第 10 章"何时记/记什么"、维护七纪律（K.7）对第 10 章"过时记忆比无知更糟"与附录 J.19 冲突治理、检索注入（K.8）对第 5 章位置策略、评测（K.10）对第 15 章与附录 N、五路线（K.11）对附录 O 产品盘点、Coding Agent 记忆（K.12）对第 23 章知识文件、Skill 晋升（K.14）对第 6 章与第 10 章毕业通道。信息基准 2026-08-31（[C-48]），发行前按附录 C 清单复核。
