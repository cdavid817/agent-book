...
### 20.7.6 AnythingLLM、Open WebUI、Kotaemon
...
## 20.8 Code-first RAG 框架

### 20.8.1 LangChain 与 LangGraph
...
### 20.11.2 AWS Bedrock Knowledge Bases
...
### 20.11.3 Azure AI Search / Microsoft Foundry
...
### 20.11.4 Google Vertex AI RAG Engine
...
## 20.42 Retrieval Text 与 Generation Text 分离
...
### 20.52.2 共享 Collection + tenant_id 过滤
...
### 20.79.1 Reciprocal Rank Fusion
...
### 20.111.6 Context Precision

相关 Chunk 是否排在不相关 Chunk 前面。

### 20.111.7 Context Recall
...
### 20.117.1 Answer Correctness

答案与标准答案、事实或业务规则是否一致。

### 20.117.2 Answer Relevance

答案是否直接回应用户问题，而不是只复述背景。

### 20.117.3 Faithfulness
...
### 20.117.4 Completeness

标准答案中的关键点是否全部覆盖。

### 20.117.5 Conciseness

是否存在大量无关、重复或未经请求的内容。该指标不能以牺牲完整性和正确性为代价。

### 20.117.6 Refusal Accuracy
...
### 20.117.7 Numerical Correctness
...
## 20.127 OpenTelemetry 与 OpenInference
...
### 20.144.1 把向量数据库当成完整 RAG

忽略数据导入、解析、权限、重排、引用和评测。

### 20.144.2 只调 Chunk Size
...
### 20.144.5 所有查询都走 Agentic RAG

简单问题被过度规划，成本和不确定性显著增加。

### 20.144.6 一开始就上 GraphRAG
...
> **使用提示**：与其他附录的分工——1 讲模型机制、2 讲方法论、3 记来源、4 列产品、5 辨异同、6 索引图版、7 详解 OTel、8 上手 DeepEval、9 评测观测平台选型、10 上手 Mem0、11 详解记忆晋升机制、12 盘点 Coding Agent 赛道、13 盘点可观测赛道、14 盘点评估赛道、15 盘点 Memory 赛道、16 盘点自进化赛道、17 盘点多 Agent 赛道、18 盘点 MCP 生态、19 盘点沙箱赛道、**20 盘点 RAG 赛道**、21 盘点 LLM Wiki 赛道、22 盘点 Loop Engineering 赛道、23 解析 Pi 源码、24 解析 Claude Code 源码、25 解析 Codex 源码、26 解析 OpenCode 源码。对照阅读：本附录与第 11 章是"手册对机制"的关系——逐环节对读：数据导入（第三部分）对第 11 章 2.1、解析分块（第四/五部分）对 2.2、Embedding（第六部分）对 2.3 与附录 1.8、向量存储与索引（第七/八部分）对 2.7、检索前处理（第九部分）对 2.4、混合检索与后处理（第十/十一部分）对 2.5、生成与引用（第十二部分）对 2.6、Agentic/GraphRAG（第十三部分）对 2.9、评估（第十四部分）对 2.8 与附录 8/14、观测治理（第十五部分）对第 14 章与附录 13、Memory 之辨对附录 5.1 与 15。信息基准 2026-08-31（[C-46]），发行前按附录 3 清单复核。
