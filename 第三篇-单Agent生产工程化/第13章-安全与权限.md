...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/ch13-安全与权限.md](../详解扩展版/ch13-安全与权限.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
...
**人在环是一组 UX 契约，不是一个确认弹窗。** 光有闸门不够，人要能看懂、拦得住、救得回，前端与审计须对齐一份能力清单（代码里的 `HITL_AFFORDANCES`）：**计划预览**与**动作预览**（含 **Diff**：改什么一目了然）、**证据面板 + 来源引用**（为什么这么做，接第 14 章 Trace 与附录 C 来源纪律）、**不确定性展示**（模型没把握时要显式标注，而非佯装笃定）、**撤销**（仅对可逆动作有效——不可逆动作的"撤销"是幻觉，必须在事前用确认拦住）、**暂停 / 取消 / 恢复**（接第 12 章 Checkpoint 与第 18 章 interrupt 语义）、**计划修正**（人改计划后 Agent 接着跑）、以及**人工接管 / 坐席 handoff**（L4 异常时把方向盘交回人手，带上完整上下文而非从零开始）。一句话：**授权是渐进的，退路是常备的**。
