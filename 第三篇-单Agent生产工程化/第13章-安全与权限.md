...
VaneHub 助手准备从试点走向全员开放，卡在了安全评审会上。安全团队没有质疑模型能力，而是提了三个工程问题，每个都直击现有实现的软肋：
...
本章增量：`src/vanehub/security/policy.py`——**策略表驱动的 PDP** + **Hook 形态的 PEP**。策略是数据（可从 JSON/DB 热加载），引擎实现 deny 优先、上下文条件与污点升级；PEP 接进第 6 章的 Hook 管线，ask 路径带超时默认拒绝与会话批准记忆。

```python
# src/vanehub/security/policy.py — 策略表驱动的 PDP
import fnmatch
import re
...
```python
# src/vanehub/security/pep.py — Hook 形态的 PEP
from vanehub.core.hooks import HookDecision
