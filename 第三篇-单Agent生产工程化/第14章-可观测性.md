...
**跨边界传播是 Agent Trace 的真难点。** Agent 的一次任务会跨进程、跨协议、跨组织——MCP 调工具、A2A 委派子 Agent、任务进消息队列/后台、浏览器执行器、人工审批、异步 resume。每跨一道边界，都要把 **W3C `traceparent`** 沿载体透传（HTTP 头 / A2A 的 `_meta` / 队列消息属性），远端的 span 才能挂回同一 `trace_id`。否则每跨一次边界 Trace 就断一次，排障退回"各查各的日志"。这与第 18 章 A2A 的 `RemoteTask.trace_context`、第 8 章 MCP 的 `_meta` OTel 键是同一件事的两端。
