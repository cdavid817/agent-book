...
周四凌晨，值班群炸了：一位运营在下班前让 VaneHub 助手批量更新 30 个工单的状态，早上发现其中 6 个被改错了字段。值班工程师打开日志，只找到几行 `session started ... session ended (ok)`——应用日志显示一切正常。接下来的三个问题，把团队按在了原地：
...
```mermaid
graph TB
    S["① Session Span（根）<br/>必带: session_id · user/tenant · task_type<br/>总 token/费用 · 最终状态(成功/熔断/失败)"]
    A1["② Agent Span（每轮一个）<br/>必带: turn 序号 · model · stop_reason<br/>input/output/cache tokens · 耗时"]
    A2["② Agent Span (turn=42)"]
    T1["③ Tool/MCP Span<br/>必带: 工具名 · effect 等级(第9章)<br/>is_error · 重试次数 · 结果字节数"]
    T2["③ Tool Span: update_ticket"]
    P1["④ Process Exec Span<br/>必带: 命令(脱敏) · 退出码<br/>沙箱标识 · 输出截断标记"]
    S --> A1
    S --> A2
    A1 --> T1
    A2 --> T2
    ...
    classDef turn fill:#C0D6DF,stroke:#4F6D7A,color:#1f2d33
    classDef tool fill:#E8DAB2,stroke:#4F6D7A,color:#1f2d33
    classDef proc fill:#DD6E42,stroke:#DD6E42,color:#ffffff
    class S root
    class A1,A2 turn
    class T1,T2 tool
    class P1 proc
...
graph TB
    ...
    subgraph E2["效率 Efficiency——代价多大"]
        M4["每任务成本 P50/P95<br/>(分 token 类别计价)"]
        M5["轮次分布分位数"]
...
**语义约定优先。** OTel 的 **GenAI 语义约定**（`gen_ai.*` 属性族：模型名、token 用量、操作类型等）已趋稳定——自定义属性名之前先查约定，用标准名的直接收益是现成工具链（各家 LLM 观测平台）零适配接入。
...
本章增量：`src/vanehub/obs/otel_bridge.py`——作为事件总线的一个消费者接入 **OTel SDK**（`pip install opentelemetry-sdk`），把事件配对成四层 Span，并落三个核心指标。GenAI 语义约定与其弃用状态以 [C-04] 所 pin 的规范为准；升级 SDK 时先跑事件契约回归。零侵入：循环代码一行不改（第 12 章"一次发射、多方消费"的兑现）。

```python
# src/vanehub/obs/otel_bridge.py — 事件流 → OTel Span/指标 桥接器
...
RESOURCE = Resource.create({"service.name": "vanehub-assistant"})
...
class OtelBridge:
    ...
    def __init__(self):
        self.tracer = trace.get_tracer("vanehub")
        meter = metrics.get_meter("vanehub")
        # 三个核心指标（2.2 节口径）
        self.tokens = meter.create_counter(
            "vanehub.tokens.used", description="token 消耗, 按类别分列")
        self.tool_calls = meter.create_counter(
            "vanehub.tool.calls", description="工具调用数, 按工具与最终结果分列")
        self.turns_hist = meter.create_histogram(
            "vanehub.session.turns", description="已结束会话的轮次分布")
