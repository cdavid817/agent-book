...
本章增量：`src/assistant/obs/otel_bridge.py`——作为事件总线的一个消费者接入 **OTel SDK**（`pip install opentelemetry-sdk`），把事件配对成四层 Span，并落三个核心指标。GenAI 语义约定与其弃用状态以 [C-04] 所 pin 的规范为准；升级 SDK 时先跑事件契约回归。零侵入：循环代码一行不改（第 12 章"一次发射、多方消费"的兑现）。
