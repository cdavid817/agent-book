...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/ch15-评测Eval.md](../详解扩展版/ch15-评测Eval.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
...
| **检索/RAG 专项** | recall@k / MRR | 黄金集上应命中文档的召回与排名 | 检索层独立归因（第 11 章 2.8） |
| | faithfulness / 引用正确率 | 答案锚定材料的比例 / 引用与断言对齐率 | 响应层独立归因（第 11 章 2.8，RAGAS 口径） |
...
| **强项** | 声明式配置、矩阵对比（多 prompt × 多模型）、轻量 | 指标库全（相关性/忠实度/毒性）、G-Eval（自然语言写 rubric 即成指标）、组件级 Agent 断言、CI 门禁体验最直接 | 多步任务、沙箱与工具支持、轨迹级评测 | faithfulness / context precision 等检索专用口径（第 11 章的独立评测正用它） |
| **适用** | 提示词迭代的日常回归 | 应用层功能点断言 | 端到端 Agent 任务评测 | 检索层与生成层分离归因 |

选定 DeepEval 后的完整上手路径（工作模型、G-Eval/DAG 写法、RAG/Agent/多轮三条实战线、CI/CD 分层门禁与阈值校准）单独成册，见**附录 H**——本章讲"评什么、为什么评"，附录 H 讲"用 DeepEval 怎么评"。五个代表项目（DeepEval/Ragas/Phoenix/Langfuse/MLflow）的逐项深度调研、横向对比与组合方案见**附录 I**——本章两张表是地图，附录 I 是地形志。

平台层的选型逻辑不同：它们把第 14 章的 Trace 与本章的评测放在同一份数据上——线上轨迹一键转评测用例（2.4 节的回流闭环有现成载体），因此**选平台首先看与观测栈的一致性**，别让评测数据和 Trace 数据分家。两层的协作形态也定型了：**框架产分、平台收分**——DeepEval/Ragas 在 CI 里算出的分数以统一 Score 模型回写平台（各平台均有 Score 接收接口），离线门禁与在线监控从此看同一份质量数据。生产环境几乎从不是"五选一"，而是"一个平台 + 一到两个框架"的组合：
...
| **注意** | 生态外使用集成成本上升 | 需自己运维（ClickHouse/Redis/对象存储四件）；Open Core——核心 MIT、企业目录商业证 | 商业闭源 | 平台功能较商业产品轻；ELv2 许可——禁止作为托管服务转售 | LLM 专项能力晚于原生平台，评测器生态较薄 |
...
**评测集是含生产数据的资产。** bad case 回流带着真实用户数据——入集前脱敏（第 14 章管道复用）；评测集本身版本化并做访问控制；每个用例带 **source 枚举**（manual / production_trace / regression_bug / synthetic / red_team——本章 `origin` 字段的规范化），集合构成从此可审计（"合成占比多少、红队覆盖几条"一查便知）；资产的**反锁定**同样要设计——用例用 JSONL 等中立格式存储、分数走统一 Score 模型、框架 SDK 只出现在 runner 适配层，迁移评测平台不丢历史（第 20 章反锁定资产条目的评测版）；用例有**时效维护**责任人：环境变了（接口迁移、SOP 更新）的过期用例要修或退役，否则"评测失败"里混着"用例过期"，信号被噪声淹没。
