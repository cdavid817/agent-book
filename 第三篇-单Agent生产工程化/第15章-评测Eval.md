...
选定 DeepEval 后的完整上手路径（工作模型、G-Eval/DAG 写法、RAG/Agent/多轮三条实战线、CI/CD 分层门禁与阈值校准）单独成册，见**附录 8**——本章讲"评什么、为什么评"，附录 8 讲"用 DeepEval 怎么评"。五个代表项目（DeepEval/Ragas/Phoenix/Langfuse/MLflow）的逐项深度调研、横向对比与组合方案见**附录 9**——本章两张表是地图，附录 9 是地形志。
