...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/详解12-运行时架构.md](../详解扩展版/详解12-运行时架构.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
...
这个实验的价值在于给全书一个正式命名：这些机制的总和叫 **Harness**——直译"马具"，即套在模型这匹马身上的全部装备。行业内的公开评测也反复印证同一结论：同一模型在不同脚手架下的 Agent 基准得分差距可达数十个百分点（SWE-bench 等榜单上，同模型不同 scaffold 的成绩差异长期存在，参见附录 2.6）。**转型 Agent 方向的工程师真正的工作物，不是模型，是 Harness**——这也是为什么本书用了整整一篇讲它的生产化。
...
这张图也是评估任何 Agent 框架/产品的检查单：逐件问"它对这一件给了什么实现、可替换性如何"——六件里缺件或不可替换的，生产化时都会变成你的工作量。市面主流产品与框架按这张检查单的逐一定位见附录 4。
