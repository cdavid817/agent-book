...
两条通道之外还有一种 2025 年起出现的形态：**模型自管记忆**——把记忆暴露为文件式工具（如 Anthropic 的 memory tool、Claude Code 的自动记忆），模型在会话中自主决定读写什么，运行时只负责跨会话持久化。它与本章的运行时判定形态可并存：自管形态把"何时记"的判断交给模型（省了会话末的提取调用，但写入质量依赖模型自觉），准入标准、写入侧脱敏与作用域隔离的纪律对两种形态同样成立。
