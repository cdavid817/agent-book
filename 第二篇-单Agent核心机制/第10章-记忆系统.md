...
> 本章另有约十倍篇幅的**详解扩展版**（深读初稿，以正文为准）：[详解扩展版/详解10-记忆系统.md](../详解扩展版/详解10-记忆系统.md)。
...
*图 1：短期/长期两级框架与 CoALA 四分类的映射——这张图回答"两套术语如何对应、每类记忆存在哪里、活多久"。两级框架回答"要不要活过会话"这一工程决策，四分类回答"落到哪种介质"；虚线通道有二：长期记忆经注入进窗口才生效，语义记忆中反复出现的做法毕业为程序记忆（Skill）。 CoALA 原始分类图见附录 6.3。*
...
*图 2：CoALA 四类记忆在一次会话中的读写流转——这张图回答"四类记忆分别在会话的哪个时点被读、被写"。读（启动注入/按需加载）发生在会话前中段，写（回写事件、提炼事实偏好）发生在会话末，"毕业为 Skill"则跨越多次会话累积——三条时序把四类记忆的分工从"是什么"讲成了"何时用"（本图为本书原创视角，CoALA 原始分类图见附录 6.3）。*
...
**为什么 SQLite 起步，而不是向量库？** 三个理由：其一，量级——单用户的记忆条目在百到千级，全量装入内存都不成问题，语义检索的召回优势没有发挥空间；其二，查询形态——记忆召回的主过滤条件是**结构化的**（作用域=这个用户+这个项目，按置信与新近度排序；过期条目不剔除、仅降级为待重验），SQL 一条语句的事；其三，运维成本为零。向量检索真正的用武之地在第 11 章的知识库场景（百万级文档、自然语言查询）；记忆库若长到需要语义召回，加一列 embedding 即可平滑升级——**不为千条数据预付百万条的架构**（第 1 章坑 5 的又一次应用）。记忆、RAG、压缩、结果截断四者的横向辨析见附录 5.1（"记忆和 RAG 什么区别"的一表答案）。
...
**远程记忆后端的三条路线。** 迁出单实例后具体落到哪，按"外包多少"分三条路线（名单为 2026 年中快照，时效纪律同附录 4——半衰期 6–12 个月，选型按第 20 章评分卡在自家负载上实测）：

| 路线 | 开源代表 | 国内厂商/云 | 买到什么 | 代价 |
|---|---|---|---|---|
| **托管记忆服务**（提取-存储-召回整段外包） | Mem0、Letta（MemGPT 谱系）、Zep / Graphiti（时序知识图谱记忆） | 阿里云百炼长期记忆、字节扣子（Coze）长期记忆与数据库、百度千帆 AppBuilder 记忆组件 | 免建提取管线与召回逻辑，接 API 即用 | "记什么"的准入被外包；平台内建型随平台绑定 |
| **数据库自建**（本章仓储换远程后端，模型平移） | Milvus（Zilliz 主导的 LF 毕业项目）、Vearch（京东）、关系库向量列：TiDB / OceanBase / PolarDB、MemoryScope（阿里，对话记忆框架） | 腾讯云向量数据库、火山引擎 VikingDB、阿里云 DashVector / AnalyticDB 向量版、百度向量数据库、Zilliz Cloud | 架构完全自主，本章 scope/TTL/置信/遗忘模型原样平移 | 提取、遗忘、注入全部自建——即本章全部内容 |
| **新形态（关注层）** | MemOS（"记忆操作系统"：记忆分层与调度的一等抽象）、Memobase（用户画像式记忆） | — | 记忆作为一等资源管理的前沿探索 | 按第 26 章三问验生产信号后再投入 |

三条选型纪律，全部回扣本章原则：**其一，先看隔离再看向量。** 记忆召回的第一条件是作用域硬过滤（WHERE 不是 ORDER BY，2.4 节）——纯向量库要靠 payload/元数据过滤自己补齐租户级硬隔离，而"关系库 + 向量列"与本章"结构化过滤为主、语义为辅"的召回形态天然契合，这正是国内多家关系库加向量能力的路线对记忆场景的意义。**其二，托管 ≠ 免责。** 托管服务省掉的是管线代码，省不掉写入准入标准与提取质量回归（2.3 节、第 15 章），更省不掉"用户可见可删"的合规义务——记忆是用户数据，删除权要能级联到远程后端；国内落地再叠加第 20 章的数据出域与部署形态审查（记忆库里恰恰是最个人化的数据）。**其三，跨实例并发语义。** "强化 +0.1""同键冲突覆盖"都是读-改-写操作，多实例下要有原子更新或乐观锁保障——这是从 SQLite 单写者迁出时最容易漏的一件事。
