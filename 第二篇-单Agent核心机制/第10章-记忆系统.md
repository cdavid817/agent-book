...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/ch10-记忆系统.md](../详解扩展版/ch10-记忆系统.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
...
**远程记忆后端的三条路线。** 迁出单实例后具体落到哪，按"外包多少"分三条路线（名单为 2026 年中快照，时效纪律同附录 D——半衰期 6–12 个月，选型按第 20 章评分卡在自家负载上实测）：
