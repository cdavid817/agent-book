...
**怎么忘——三种机制。** **TTL 过期**：事实类默认 90 天（过期不删除而是降级为"待重验"）；**容量淘汰**：按 `置信度 × 新近度` 打分（`use_count` 记录使用次数、暂不计入淘汰评分，可按需纳入），每作用域限额（如 200 条），超限淘汰最低分——LRU 的记忆版；**冲突覆盖**：同一键的新条目覆盖旧条目（用户改了偏好），旧版本保留历史链而非物理删除（审计需要，也防"覆盖错了"不可逆）。
