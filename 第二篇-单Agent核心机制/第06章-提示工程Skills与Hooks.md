...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/ch06-提示工程Skills与Hooks.md](../详解扩展版/ch06-提示工程Skills与Hooks.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
...
分层的第二个理由是**缓存对齐**：提示缓存按前缀匹配，越稳定的内容越应靠前——身份层在最前、工作流层在最后，这样高频迭代只失效最短的前缀尾部，缓存命中率损失最小（缓存机制参见第 5 章与附录 A.1）。
