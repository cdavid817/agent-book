...
为了让 VaneHub 助手能自助排障（跑诊断脚本、检索日志、验证修复），团队加了一个 `run_bash` 工具——直接在应用服务器上执行命令，毕竟"它只是跑跑 grep"。两周内发生了两起性质完全不同的事故。
...
**第一层：容器/虚拟化隔离（Container Isolation）。** 用 Linux namespace + cgroup（容器）或 microVM（gVisor、Firecracker——多租户场景用后者，因为共享内核的容器逃逸 CVE 每年都有新增）把执行环境与宿主隔开。拦截的威胁：**宿主逃逸**（进程看不到宿主文件系统与进程表）、**资源耗尽**（cgroup 限 CPU/内存/PID 数——fork 炸弹和内存泄漏被限在格子里）、**网络外传**（默认断网，出口走白名单代理——事故二的 `curl` 在这一层直接连接失败）。网络策略值得单独强调：**默认拒绝出站**是对抗数据外传（Exfiltration）的最有效单点，因为注入可以说服模型做任何事，唯独说服不了防火墙。
...
```mermaid
graph TB
    subgraph L1["第一层: 容器/microVM 隔离"]
        T1["拦截: 宿主逃逸<br/>资源耗尽(fork炸弹/OOM)<br/>网络外传(默认断网+出口白名单)"]
    end
    subgraph L2["第二层: 权限降级"]
        T2["拦截: 提权链<br/>系统文件篡改<br/>(非root · capability裁剪 · 只读rootfs)"]
    end
    subgraph L3["第三层: 文件系统白名单"]
        T3["拦截: 越权读(密钥/他人工作区)<br/>越权写(共享目录)<br/>(仅挂工作区 · realpath校验)"]
    end
    CODE["Agent 生成的命令/代码<br/>(不可信输入)"] --> L3 --> L2 --> L1
    L1 --> HOST["宿主与企业内网<br/>(受保护资产)"]

    classDef threat fill:#DD6E42,stroke:#DD6E42,color:#ffffff
    classDef layer fill:#C0D6DF,stroke:#4F6D7A,color:#1f2d33
    classDef asset fill:#4F6D7A,stroke:#4F6D7A,color:#ffffff
    class CODE threat
    class T1,T2,T3,L1,L2,L3 layer
    class HOST asset
```
...
一个设计推论：**给非 Coding 场景的 Agent 也配一个工作区**。VaneHub 的审计报告、中间数据、抓取的日志都写进会话工作区——人可以中途打开看、改、接管，这种"人机共享现场"的协作模式是纯对话式交互给不了的。
...
本章增量：`src/vanehub/tools/shell.py`——**带白名单与副作用分级的命令执行器**。它是三层防线中第三层的进程级简化实现（生产环境外面还要套容器层，见第 4 节）；分级标注随结果返回，为第 13 章的策略接入预留接口。

```python
# src/vanehub/tools/shell.py — 白名单命令执行器 + 副作用分级
import shlex
import subprocess
...
registry.register(RegisteredTool(
    ...
    input_schema={"type": "object",
                  ...
                  "required": ["command"]},
    run=lambda inp: (lambda r: r.output + f"\n[操作等级: {r.effect}]")
                    (shell.run(inp["command"])),
    idempotent=False))
```
