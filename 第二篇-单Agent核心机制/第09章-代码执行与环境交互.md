...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/详解09-代码执行与环境交互.md](../详解扩展版/详解09-代码执行与环境交互.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
...
"工作区 + 沙箱执行"还派生出第 7 章 2.6 节预告的**代码模式调用（Code Mode）**：把工具暴露为代码 API（一个可 import 的函数库），模型不再逐个发 `tool_use`，而是写一段脚本在沙箱里完成过滤、循环与多工具编排——中间数据在文件系统与变量间流转，**不经过上下文**，只有最终结论作为观察返回。收益正是本章两个机制的乘积：沙箱让"执行任意编排代码"变得可授权（边界硬了才敢放权），工作区让中间产物可见可审（脚本与数据都落盘）。代价是调试面转移——错误从"一次工具调用失败"变成"一段脚本的运行时异常"，错误回喂要带上脚本的 traceback 摘要（第 7 章三要素同样适用）。
...
### 2.5 Browser / Computer Use：GUI 环境的安全动作循环

代码执行的环境是文件系统与 shell；另一类环境是 **GUI**——浏览器与桌面（Computer Use）。它比工具调用凶险得多，因为**环境不受你控制、随时在变**。先看感知：同一个页面有三种读法，稳定性递减——**DOM**（结构化、选择器可定位）、**Accessibility Tree**（语义角色，跨渲染较稳，定位首选）、**Screenshot**（像素，最通用但坐标最易漂）。能用 AXTree/DOM 定位就别只靠截图坐标。

GUI 的两个致命坑：**坐标漂移**（截图定位的按钮，页面一重排就点到别处）与**页面状态过期**（定位时是登录页，点下去时已跳转）。二者根因相同——**观察与执行之间隔了时间，页面变了**。解药是把执行拆成七段、每段拒绝盲动的循环：

> Observe → Ground → Propose → Policy Check → Execute → Verify → Commit / Rollback

两条守卫最救命：**过期守卫**（执行前重新观察，页面版本若已变就拒绝按旧坐标动作、退回重看）与**动作后验证 + 回滚**（点完要断言"真到了预期状态"，不符即回滚——别把"点了"当"成了"）。高风险动作（上传/下载/提交表单/支付）先过策略闸（第 13 章 PDP 与自治级别）：

<!-- snippet: examples/reference-assistant/src/assistant/browser/loop.py#ch09-browser-loop mode=executable verified_by=examples/reference-assistant/tests/test_browser.py -->
```python
def run_step(
    *,
...
其余 GUI 纪律按同一思路配齐：**Tab / 窗口 / 弹窗**要显式管理焦点（弹窗劫持是常见卡死点）；**上传/下载**是跨信任边界的文件流，按第 2.4 节副作用分级 + 供应链纪律处置；**页面即注入面**——抓取到的页面内容是不可信数据，第 5 章三层防御原样适用（一个恶意页面能在文本里藏指令）；**隐私字段遮蔽**——截图/录屏证据里的密码、卡号要遮蔽（第 14 章 §2.6 采集分级的 GUI 版）；**高风险动作留视频/截图证据**便于事后审计。**Voice Agent** 另有一套延迟预算：VAD（语音活动检测）判断说话起止、支持**打断**（barge-in），端到端延迟要卡在对话可接受的预算内（通常几百毫秒），否则再准也没人等。计算机操作类能力的公共基准（OSWorld / WebArena）见 [C-05]，评测口径同样是"多次采样看通过率"（第 15 章）。
...
class SandboxedShell:
    ...
    def _paths_confined(self, argv: list[str]) -> bool:
        ...
        已知盲区：'-' 开头的参数被整体跳过，'--output=/etc/x' 这类"选项=路径"
        形式逃过本层校验——由外层 bind mount 兜底（区外路径物理不存在，见第 4 节）"""
        for arg in argv[1:]:
            if arg.startswith("-"):
                continue
...
*修复*：默认断网 + 出口白名单代理（依赖镜像、允许的内部 API）；`curl` 类命令分级为不可逆走审批；出口代理全量记录，作为外传检测的数据源。还要补一刀：**白名单里可写入的公共服务同样是外传通道**——代码托管、制品库、issue 系统都能"合法地"带走一个含密钥的文件，注入完全可以诱导这类提交；因此白名单尽量只放只读端点，可写端点的流量纳入外传检测而非默认信任。
