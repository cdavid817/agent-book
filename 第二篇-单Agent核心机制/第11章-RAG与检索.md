...
> 第二篇 D. 记忆与检索（状态层）
>
> 第 10 章解决"这个 Agent 记得什么"，本章解决"组织的知识库里有什么"。主线是一个角色转变：**检索从传统 RAG 的"前置管线"降格为 Agent 的"工具之一"**——这个降格反而让它变强了。本章沿"入库 → 检索 → 生成"走完 RAG 全链：入库管线、检索层技术选型（混合/重排/查询侧优化）、生成层的可信答案机制（组装/忠实度/引用/冲突消解）、两个重要变体，以及一个反直觉结论：Coding 场景下 grep 为何反超向量检索。
