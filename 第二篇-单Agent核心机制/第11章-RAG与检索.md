...
> 第二篇 D. 记忆与检索（状态层）
>
> 第 10 章解决"这个 Agent 记得什么"，本章解决"组织的知识库里有什么"。主线是一个角色转变：**检索从传统 RAG 的"前置管线"降格为 Agent 的"工具之一"**——这个降格反而让它变强了。本章覆盖入库管线、检索层技术选型、两个重要变体，以及一个反直觉结论：Coding 场景下 grep 为何反超向量检索。

---

## 1. 场景引入：一条经典 RAG 管线的四种死法

VaneHub 助手要接入运维知识库（1.2 万篇 SOP、故障复盘、架构文档）。团队按 2023 年的标准做法搭了**传统 RAG 管线**：用户提问 → 向量化 → 召回 top-5 片段 → 拼进上下文 → 生成答案。上线一个月，评测成功率 61%，失败案例归为四类，每类都指向管线的一个结构性假设：
...
代价也要说清：Agentic 检索平均多 1–3 轮 LLM 调用，延迟与成本上升。因此选型延续第 1 章 2.5 节的判据：**查询形态可预测的高频简单问答（FAQ 类），传统管线仍是正解**（一次调用、延迟可控）；查询开放、需要多跳与纠错的场景才值得 Agentic 化。VaneHub 的重构采用混合：FAQ 命中走旧管线，未命中降级进 Agent 循环——成功率 61% → 83%，其中约三分之二的提升来自"追加检索"回路。
...
选型纪律与第 10 章同款：**从量级出发，不为想象中的规模预付运维成本**——VaneHub 的 1.2 万篇文档，Chroma 甚至 SQLite+numpy 都绰绰有余。
...
flowchart TB
    ...
    D1 -->|"是, 且语料是代码/日志"| GREP["Grep/文件系统检索<br/>(2.5 节)"]
...
*图 3：检索方案选型决策树——这张图回答"什么问题形态导向什么检索方案"。默认答案是标准混合检索；三个分支都是"特定问题形态 + 愿付对应成本"才成立的例外。*

### 2.5 反直觉结论：Coding 场景下 grep 反超向量检索
...
还有第四个常被忽略的原因：**Agentic 循环补齐了 grep 的传统短板**。grep 的弱点是"不知道该搜什么词"——而循环中的模型会自己换词、加通配、缩小目录多轮尝试（2.1 节的追加检索回路），这恰是传统 RAG 时代 grep 输给向量检索的那个理由。边界同样要说清：这个结论限于**代码与结构化文本**；自然语言文档（需求文档、用户反馈）的同义改写与跨语言匹配仍是向量检索的主场——图 3 决策树的第一个分支问的就是这条边界。

---

## 3. 动手实现（贯穿项目增量）

本章增量：`src/vanehub/tools/doc_search.py`——为项目文档实现**混合检索工具**（BM25 + 词袋余弦双路召回、RRF 融合、结构感知切分）。零外部依赖以保持可运行：稠密通道用词袋余弦作**教学替身**（接口与真 embedding 一致，生产替换为 embedding API 即可）；中文用字符 bigram 近似分词（生产换真分词器）。

```python
# src/vanehub/tools/doc_search.py — 混合检索：结构感知切分 + BM25/余弦双路 + RRF
import math
import re
...
def build_doc_search(docs_dir: str):
    ...
    def run(inp: dict) -> str:
        ...
        lines = [f"共 {len(hits)} 条命中（可根据需要换词追加检索）："]
        for h in hits:
            lines.append(f"--- [{h['section']}] 来源: {h['doc']}\n{h['text']}")
        return "\n".join(lines)
...
回测四种死法：死法一、二由循环回路解决（模型可拆查、可换词重查——工具描述明确鼓励）；死法三由描述里的"何时不用"抑制；死法四由 `chunk_markdown` 的结构感知（表格行与代码块不作为切点）缓解。BM25 通道保证 `ERR-4032` 这类精确查询命中，余弦通道兜住同义改写——两路互补正是 2.3 节的设计落地。
...
**检索质量单独评测。** 端到端成功率混合了检索与生成两层的质量，问题定位需要拆开：检索层用 **recall@k / MRR** 独立评测（标注"问题→应命中文档"的黄金集），生成层固定检索结果评生成。经验：端到端失败案例中检索层责任占比常超一半，而团队的优化精力往往错配在提示词上（评测体系参见第 15 章）。
