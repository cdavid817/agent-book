...
> 本章另有约十倍篇幅的**详解扩展版**：[详解扩展版/详解18-编排通信与图执行.md](../详解扩展版/详解18-编排通信与图执行.md)——独立成文的深读版（初稿，未经审读与来源核验，体例与正文不同；两版内容以正文为准）。
