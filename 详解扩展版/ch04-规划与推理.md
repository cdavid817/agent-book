...
> 本文基于 `awesome-agent-tutorial` 原章节《第 04 章：规划与推理》重新组织并大幅扩展。原文提出了隐式规划、显式规划、TODO 驱动和 ReWOO 四种取向，并讨论了搜索式推理、反思、重规划与推理预算。扩展版进一步补充：概念边界、计划数据模型、状态机、DAG 并行调度、Planner/Executor/Verifier/Replanner 分层、权限审批、持久化恢复、可观测性、评估体系、完整代码骨架和面试题。
...
### A.10 Search Value Evaluator 模板
