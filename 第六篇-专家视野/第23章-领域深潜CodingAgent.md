...
**按需导航**：地图之后的精确定位交给两件已备好的工具——**Grep**（精确符号与字面量检索，第 11 章 2.5 节论证过它为何在 Coding 场景反超向量检索：精确、实时、零索引）与 **LSP**（definition/references/diagnostics 的符号级理解，第 9 章 2.3 节）。三者的分工可以一句话记住：**Map 管"大概在哪片"，Grep 管"具体在哪行"，LSP 管"和谁有关系"**。
