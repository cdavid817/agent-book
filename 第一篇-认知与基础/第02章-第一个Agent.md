...
这道题考察的不是编码量，而是**心智模型**：你是否理解 Agent 的本质只是"一个调用 LLM API 的 while 循环，循环体里执行模型点名的工具"。理解了这一点，任何框架都只是这个循环的封装变体；不理解这一点，用框架就是在黑盒上叠魔法。本章我们就把这道面试题完整做一遍——写完你会发现，**核心循环不到 30 行**，其余代码都在做工具注册和 HTTP 拼装这类"体力活"。
...
- 成本问题：n 轮任务付 n 次全量历史的输入费用，缓解手段是提示缓存（参见附录 A.1）。
