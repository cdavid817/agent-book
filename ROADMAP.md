...
**已完成**：13.6 统计可靠的评测门禁（gate.py + 测试 + 第 15 章 2.6）；13.5 安全标准映射（第 13 章 2.6 + C-16~C-19）；13.2 Durable Execution（executor.py + 测试 + 第 12 章 §4）；13.1 MCP/A2A 互操作（a2a.py + 测试 + 第 18 章 2.7）；13.4 人机交互与自治级别（autonomy.py + 测试 + 第 13 章 2.7）；13.7 扩展供应链（registry.py + 测试 + 第 8 章 2.7）；13.8 观测增强（spans.py + 测试 + 第 14 章 2.6）。

**待推进**：13.3 多模态/Browser/Computer Use。
